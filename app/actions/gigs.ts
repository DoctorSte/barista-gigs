...
export async function createGig(
  ...
  const { startsAt, endsAt } = shiftSpan(parsed.data.shifts);
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      title: parsed.data.title || defaultTitle(parsed.data.shifts),
...
export async function updateGig(
  ...
  revalidatePath("/shop/dashboard");
  revalidatePath(`/shop/gigs/${gigId}`);
  return { ok: true };
}
...
export async function decideInterest(
    ...
    if (extraProfile && gig) {
      if (decision === "accepted") {
        ...
        await notify(extraProfile.user_id, {
          ...
          body: "The shop went with someone else this time.",
  ...
  revalidatePath(`/shop/gigs/${interest.announcement_id}`);
