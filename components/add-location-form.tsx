...
export function AddLocationForm({ featuredCities }: { featuredCities: City[] }) {
  ...
  return (
    <form action={action} className="flex flex-col gap-5">
      <Field label={d.shopForm.locationName} error={error?.field === "shopName" ? error.error : undefined}>
      ...
      <SubmitButton className="self-start">{d.cafe.addLocation}</SubmitButton>
    </form>
  );
}
